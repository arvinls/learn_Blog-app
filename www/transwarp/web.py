...
class UTC(datetime.tzinfo):
	...
	def __init__(self, utc):
		...
		mt = _RE_TZ.match(utc)
		if mt:
			minus = mt.group(1)=='-'  #True or False
			h = int(mt.group(2))
			m = int(mt.group(3))
			if minus:
				h, m = (-h), (-m)
...
def view(path):
	...
	Traceback (most recent call last):
		...
	ValueError: Except return a dict when using @view() decorator.
	'''
	def _decorator(func):
...
_RE_INTERCEPTROT_ENDS_WITH = re.compile(r'^\*([^\*\?]+)$')

def _build_pattern_fn(pattern):
	m = _RE_INTERCEPTROR_STARTS_WITH.match(pattern)
	if m:
		return lambda p: p.startswith(m.group(1))
	m = _RE_INTERCEPTROT_ENDS_WITH.match(pattern)
	if m:
		return lambda p: p.endswith(m.group(1))
	raise ValueError('Invalid pattern definition in add_interceptor.')
	...
	def get_wsgi_application(self, debug = False):
		...
		def wsgi(env, start_response):
			...
			try:
				...
				logging.exception(e)
				if not debug:
					start_response('500 Interna Server Error', [])
